...
// Code Example of using XDOM 4.1.4
// Delphi implementation
//
// You need XDOM 4.1.4 or above to use this source code.
// The latest version of XDOM can be found at "http://www.philo.de/xml/".
//
// This example source code shows how to use XDOM's XPath functions.
...
begin
  ...
  try
    ...
    Root:= Treeview.Items.AddObject(nil, Concat(Doc.NodeName,' (',GetEnumName(TypeInfo(TdomNodeType), Integer(Doc.NodeType)),') ',Doc.Classname), Doc);
...
var
  ParsingTime, TransformationTime, UpTime, ValidationTime: {$ifdef VER100} Integer; {$else} Cardinal; {$endif} // use cardinal in D4, D5, and D6
  Doc_1: TdomDocument;
...
begin
  ...
  if TabSet1.TabIndex > -1 then begin
    ...
    UpdateTreeView(TDomCustomDocument(TabSet1.Tabs.Objects[TabSet1.TabIndex]));
    Update;
  end;
end;
...
begin
  Caption:= 'XDOM ' + DomImplementation.xdomVersion + ' -- XPath Test Example';
end;

procedure TMainpage.EvaluateExpression(Sender: TObject);
var
  UpTime: {$ifdef VER100} Integer; {$else} Cardinal; {$endif} // use cardinal in D4+
  I: Integer;
begin
...
// Here the variabel $XDOM-version, which is case-sensitive, is bound to
// the current XDOM version as a string value.
begin
