...
uses
  DK.Xdom_4_1.XdomCore,
  Buttons, Controls, ComCtrls, Dialogs, ExtCtrls, Forms, StdCtrls,
  Tabs, System.ComponentModel;

type
  TMainpage = class(TForm)
    ParsingTimeLabel: TLabel;
    Label4: TLabel;
...
uses
  Messages, SysUtils, TypInfo, Windows;
...
{$IFDEF CLR}
  try
    IniDir := ExtractFileDir(Label1.Caption);
  except
    IniDir := '';
  end;
{$ELSE}
  IniDir := ExtractFileDir(TestsuitePathLabel.Caption);
{$ENDIF}
  if OpenDialog1.Execute then begin
    Update;

    if not FileExists (OpenDialog1.FileName) then begin
      ParsingTimeLabel.Caption := '';
      SerializationTimeLabel.Caption := '';
...
begin
  ...
  if TabSet1.TabIndex > -1 then begin
    ...
    DurationStr := Format('%d ms', [GetTickCount - UpTime]);
    SerializationTimeLabel.Caption := DurationStr;
    with Memo1 do begin
      Text := S;
      Update;
    end;
