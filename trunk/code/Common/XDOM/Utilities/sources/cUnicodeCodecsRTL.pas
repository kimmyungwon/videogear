{                                                                              }
{                             Unicode codecs v3.13                             }
{                                                                              }
{                    This unit is copyright © 2002-2007 by                     }
{                      David J Butler and Dieter Köhler.                       }
{                                                                              }
{                  This unit is part of Delphi Fundamentals.                   }
{               Its original file name is cUnicodeCodecsRTL.pas                }
{       The latest version is available from the Fundamentals home page        }
{                     http://fundamentals.sourceforge.net/                     }
...
{                                                                              }
{ The Original Code is "cUnicodeCodecsRTL.pas".                                }
{                                                                              }
...
{ 19/09/2004  3.12a Small revisions.                                           }
{ 28/01/2005  3.12b Small revisions.                                           }
{ 03/12/2007  3.13  Split into Win32 and .NET version                          }
{                   TUCS2Codec replaced by TUCS2BE and TUCS2LE.                }
{                                                                              }

{$INCLUDE cDefines.inc}

{$IFDEF VER160}
  {$DEFINE D8_D2005_D2006_ONLY}
{$ENDIF}
{$IFDEF VER170}
  {$DEFINE D8_D2005_D2006_ONLY}
{$ENDIF}
{$IFDEF VER180}  // VER180 indicates D2006 and D2007.
  {$DEFINE D8_D2005_D2006_ONLY}
{$ENDIF}
{$IFDEF VER185}  // VER185 indicates D2007 only.
  {$UNDEFINE D8_D2005_D2006_ONLY}
{$ENDIF}

unit cUnicodeCodecsRTL;

interface

uses
  System.Text, SysUtils, Windows;

const
  UnitName      = 'cUnicodeCodecsRTL';
  UnitVersion   = '3.13';
  UnitCopyright = 'Copyright (c) 2002-2007 David J Butler and Dieter Köhler';
...
function  UCS4CharToUTF8(const Ch: UCS4Char): TBytes;
function  WideCharToUTF8(const Ch: WideChar): TBytes;
...
{                                                                              }
function UCS4CharToUTF16BE(const Ch: UCS4Char): TBytes; overload;
function UCS4CharToUTF16LE(const Ch: UCS4Char): TBytes; overload;
...
function  DetectUTF8BOM(const Buf: TBytes): Boolean;
...
function  WideBufToUTF8String(const Buf: TBytes): AnsiString;
function  WideStringToUTF8String(const S: WideString): AnsiString;



{                                                                              }
{ UTF-16 byte order mark functions                                             }
{                                                                              }
const
  UTF16BOMSize = 2;

function  DetectUTF16BEBOM(const Buf: TBytes): Boolean;
function  DetectUTF16LEBOM(const Buf: TBytes): Boolean;
function  DetectUTF16BOM(const Buf: TBytes;
          out IsLittleEndian: Boolean): Boolean;
...
type
  ...
  protected
    ...
    function  ReadBuffer(var Buf: array of Byte; Count: Integer): Boolean;
    procedure WriteBuffer(const Buf: array of Byte; Count: Integer);
  ...
  public
    ...
    constructor CreateEx(const AErrorAction: TCodecErrorAction = eaException;
                ...
                const AWriteLFOption: TCodecWriteLFOption = lwCRLF);     // d.k. xxx lwLF, cf. WriteLFOption default ???

    procedure Decode(const Source: TBytes; const SourceIndex: Integer;
              const Dest: TBytes; const DestIndex: Integer;
              out ProcessedBytes, DestLength: Integer); virtual; abstract;
    function  Encode(const S: WideString;
              out ProcessedChars: Integer): AnsiString; virtual; abstract;

    function  DecodeBytes(const Bytes: TBytes): WideString;
...
{                                                                              }
function  GetSystemEncodingName: string; platform;
function  GetSystemEncodingCodecClass: TUnicodeCodecClass; platform;
...
{ Encoding detection                                                           }
{                                                                              }
function  DetectUTFEncoding(const Buf: TBytes;
...
{                                                                              }
type
  TUCS2BECodec = class(TCustomUnicodeCodec)
  protected
    procedure InternalReadUCS4Char(out C: UCS4Char;
              out ByteCount: Integer); override;
    procedure InternalWriteUCS4Char(const C: UCS4Char;
...
{                                                                              }
type
  TUCS2LECodec = class(TCustomUnicodeCodec)
  protected
    procedure InternalReadUCS4Char(out C: UCS4Char;
              out ByteCount: Integer); override;
    procedure InternalWriteUCS4Char(const C: UCS4Char;
...
{ UCS4CharToUTF8 transforms the UCS4 char Ch to a sequence of bytes in         }
{ UTF-8 encoding.                                                              }
function UCS4CharToUTF8(const Ch: UCS4Char): TBytes;
begin
  ...
  if Ch < $800 then // 2-byte sequence
    begin
      SetLength(Result, 2);
      Result[0] := $C0 or Byte(Ch shr 6);
      Result[1] := $80 or (Ch and $3F);
    end else
  if Ch < $10000 then // 3-byte sequence
    begin
      SetLength(Result, 3);
      Result[0] := $E0 or Byte(Ch shr 12);
      Result[1] := $80 or ((Ch shr 6) and $3F);
      Result[2] := $80 or (Ch and $3F);
    end else
  if Ch < $200000 then // 4-byte sequence
    begin
      SetLength(Result, 4);
      Result[0] := $F0 or Byte(Ch shr 18);
      Result[1] := $80 or ((Ch shr 12) and $3F);
      Result[2] := $80 or ((Ch shr 6) and $3F);
      Result[3] := $80 or (Ch and $3F);
    end
  else
...
{ UCS4CharToUTF16BE transforms the UCS4 char Ch to UTF-16BE encoding. The      }
{ resulting array is either 2 or 4 bytes long, depending on whether surrogat   }
{ pairs are used or not.                                                       }
function UCS4CharToUTF16BE(const Ch: UCS4Char): TBytes;
var
  C, HighSurrogate, LowSurrogate: TBytes;
begin
  case Ch of
  $00000000..$0000D7FF, $0000E000..$0000FFFF:
    begin
      C := System.BitConverter.GetBytes(Ch);
      // Remark: System.BitConverter.GetBytes returns the bytes in little endian
      //         mode, viz. $AABBCCEE returns the byte sequence $EE, $CC, $BB, $AA.
      SetLength(Result, 2);
      Result[0] := C[1];
      Result[1] := C[0];
    end;
  $0000D800..$0000DFFF:
    raise EConvertError.CreateFmt(SInvalidCodePoint, [Ch, 'Unicode']);
  $00010000..$0010FFFF:
    begin
      SetLength(Result, 4);
      HighSurrogate := System.BitConverter.GetBytes($D7C0 + (Ch shr 10));
      LowSurrogate := System.BitConverter.GetBytes($DC00 xor (Ch and $3FF));
      Result[0] := HighSurrogate[1];
      Result[1] := HighSurrogate[0];
      Result[2] := LowSurrogate[1];
      Result[3] := LowSurrogate[0];
    end;
...
{ UCS4CharToUTF16BE transforms the UCS4 char Ch to UTF-16LE encoding. The      }
{ resulting array is either 2 or 4 bytes long, depending on whether surrogat   }
{ pairs are used or not.                                                       }
function UCS4CharToUTF16LE(const Ch: UCS4Char): TBytes;
var
  C, HighSurrogate, LowSurrogate: TBytes;
begin
  case Ch of
  ...
  $00010000..$0010FFFF:
    begin
      ...
      HighSurrogate := System.BitConverter.GetBytes($D7C0 + (Ch shr 10));
      LowSurrogate := System.BitConverter.GetBytes($DC00 xor (Ch and $3FF));
      Result[0] := HighSurrogate[0];
      Result[1] := HighSurrogate[1];
      Result[2] := LowSurrogate[0];
      Result[3] := LowSurrogate[1];
    end;
  else // out of UTF-16 range
    raise EConvertError.CreateFmt(SCannotConvertUCS4, [Ch, 'UTF-16BE']);
  end;
...
begin
  ...
  N := Encoding.UTF8.GetByteCount(Buf);
  SetLength(Result, N);
  M := 0;
  L := Length(Buf);
  I := 0;
  Q := 0;
  repeat
    UCS4 := Buf[I] + Buf[I + 1] * 256;
    UTF8_Bytes := UCS4CharToUTF8(UCS4);
    J := Length(UTF8_Bytes);
    for K := 0 to J - 1 do begin // Copy to Result
      Inc(Q);
      Result[Q] := AnsiChar(UTF8_Bytes[K]);
    end;
    Inc(I, 2);
    Inc(M, J);
  until I >= L;
...
function WideStringToUTF8String(const S: WideString): AnsiString;
begin
  Result := WideBufToUTF8String(WideBytesOf(S));
end;
...
function DetectUTF16BEBOM(const Buf: TBytes): Boolean;
begin
  if not Assigned(Buf) or (Length(Buf) < 2) then
    Result := False
...
{ IsLittleEndian is True if it was detected that the UTF-16 data is little     }
{ endian order and False otherwise.                                            }
function DetectUTF16BOM(const Buf: TBytes; out IsLittleEndian: Boolean): Boolean;
begin
  if not Assigned(Buf) or (Length(Buf) < 2) then
    begin
...
begin
  for I := 0 to UnicodeCodecAliasEntries - 1 do
    begin
      A := UnicodeCodecAliasList[I].Table;
      for J := 0 to UnicodeCodecAliasList[I].Count - 1 do
        begin
{$IFDEF D8_D2005_D2006_ONLY}
          if System.String.Compare(CodecAlias, A[J], True) = 0 then
{$ELSE}
          if System.String.Compare(CodecAlias, A[J], StringComparison.OrdinalIgnoreCase) = 0 then
{$ENDIF}
            begin
...
begin
  for I := 0 to UnicodeCodecAliasEntries - 1 do
    if UnicodeCodecAliasList[I].Codec = CodecClass then
      begin
        Result := UnicodeCodecAliasList[I].Table[0];
        Exit;
      end;
  Result := '';
end;
...
begin
  Result := EncodingToUTF16(CodecClass, BytesOf(S));
end;
...
var
  E : EUnicodeCodecException;
begin
  E := EUnicodeCodecException.Create(System.String.Format(Msg, Args));
  E.ProcessedBytes := ProcessedBytes;
end;
...
function TCustomUnicodeCodec.DecodeBytes(const Bytes: TBytes): WideString;
var
  Index_1, Index_2: Integer;
  I, J, L: Integer;
  Dest: TBytes;
begin
  ...
  Index_2 := 0;
  repeat
    Decode(Bytes, Index_1, Dest, Index_2, I, J);
    Inc(Index_1, I);
    Inc(Index_2, J);
  until Index_1 = L;

  Result := System.Text.Encoding.Unicode.GetString(Dest, 0, Index_2);
end;
...
function TCustomUnicodeCodec.ReadBuffer(var Buf: array of Byte; Count: Integer): Boolean; 
begin
  Result := False;
...
procedure TCustomUnicodeCodec.WriteBuffer(const Buf: array of Byte; Count: Integer);
begin
...
procedure TCustomSingleByteCodec.Decode(const Source: TBytes;
    const SourceIndex: Integer; const Dest: TBytes; const DestIndex: Integer;
    out ProcessedBytes, DestLength: Integer);
var
  W: Word;
  C, I, L: Integer;
begin
  C := (Length(Dest) - DestIndex) div 2;
  if (Length(Source) = 0) or not Assigned(Dest) or (C <= 0) then
    begin
      ProcessedBytes := 0;
      DestLength := 0;
      Exit;
  ...
  ProcessedBytes := 0;
  L := 0;
  for I := SourceIndex to Length(Source) - 1 do
    try
      if L >= C then
        Break;
      Inc(ProcessedBytes);
      W := Ord(DecodeChar(AnsiChar(Source[I])));
      Dest[DestIndex + L] := W and $FF;   // Low byte of W.
      Dest[DestIndex + L + 1] := W shr 8; // High byte of W.
      Inc(L, 2);
    except
      on E : Exception do
        case FErrorAction of
          eaException:
            RaiseUnicodeCodecException(E.Message, I);
          eaStop:
            Break;
          eaSkip:
            ; // do not increase L
          eaIgnore:
            begin
              Dest[DestIndex + L] := Source[I];
              Dest[DestIndex + L + 1] := 0;
              Inc(L, 2);
            end;
          eaReplace:
            begin
              W := Ord(FDecodeReplaceChar);
              Dest[DestIndex + L] := W and $FF;   // Low byte of W.
              Dest[DestIndex + L + 1] := W shr 8; // High byte of W.
              Inc(L, 2);
            end;
...
function TCustomSingleByteCodec.Encode(const S: WideString;
    ...
    try
      ...
      on E : Exception do
        case FErrorAction of
          ...
          eaIgnore:
            begin
              C := AnsiChar(Ord(S[I]) and $FF);  // Use only low byte.
              Inc(L);
              Inc(M);
              Result[M] := C;
            end;
          eaReplace:
            begin
              Inc(L);
              Inc(M);
              Result[M] := FEncodeReplaceChar;
            end;
        end;
    end;
  if Length(Result) <> M then
    SetLength(Result, M);
  ProcessedChars := L;
end;
...
begin
  if ReadBuffer(Buf, 1) then
    begin
      C := Ord(DecodeChar(AnsiChar(Buf[0])));
      ByteCount := 1;
    end
  else
    begin
...
{ UTF-8                                                                        }
{                                                                              }
procedure TUTF8Codec.Decode(const Source: TBytes; const SourceIndex: Integer;
    const Dest: TBytes; const DestIndex: Integer;
    out ProcessedBytes, DestLength: Integer);
var
  L, M, N, O: Integer;
  ...
  W: Word;
begin
  N := (Length(Dest) - DestIndex) div 2;
  if (Length(Source) = 0) or not Assigned(Dest) or (N <= 0) then
    begin
      ProcessedBytes := 0;
      DestLength := 0;
      Exit;
  ...
  ProcessedBytes := 0;
  L := Length(Source) - SourceIndex;
  M := 0;
  repeat
    if M >= N then
      Break;
    try
      S := AnsiEncoding.GetString(Source, SourceIndex + ProcessedBytes, L);
      Error := UTF8ToWideChar(S, L, O, C);
      case Error of
        UTF8ErrorNone:
          begin
            W := Ord(C);
            Dest[DestIndex + M * 2] := W and $FF;   // Low byte of W.
            Dest[DestIndex + M * 2 + 1] := W shr 8; // High byte of W.
            Inc(M);
            Inc(ProcessedBytes, O);
            Dec(L, O);
          end;
        UTF8ErrorInvalidEncoding:
        ...
        raise EConvertError.CreateFmt(SUTF8Error, [Ord(Error)]);
      end;
    except
      on E : Exception do
        case FErrorAction of
          eaException:
            RaiseUnicodeCodecException(E.Message, ProcessedBytes);
          eaStop:
            Break;
          eaSkip:
            begin
...
function TUTF8Codec.Encode(const S: WideString;
    out ProcessedChars: Integer): AnsiString;
var
  B: TBytes;
  I, J, L, M: Integer;
begin
  ...
  for I := 1 to Length(S) do
    begin
      B := WideCharToUTF8(S[I]);
      L := Length(B);
      for J := 0 to L - 1 do
        Result[M + J] := AnsiChar(B[J]);
      Inc(M, L);
      Inc(ProcessedChars);
    end;
...
procedure TUTF8Codec.InternalReadUCS4Char(out C: UCS4Char;
    out ByteCount: Integer);
const
  MaxCode: array[1..6] of UInt32 =
      ($7F, $7FF, $FFFF, $1FFFFF, $3FFFFFF, $7FFFFFFF);
var
  Buf: array[0..0] of Byte;
  B, First, Mask: Byte;
begin
  if not ReadBuffer(Buf, 1) then
    begin
      C := UCS4_STRING_TERMINATOR;
      ByteCount := 0;
      Exit;
...
{ UTF-16BE                                                                     }
{                                                                              }
procedure TUTF16BECodec.Decode(const Source: TBytes; const SourceIndex: Integer;
    const Dest: TBytes; const DestIndex: Integer;
    out ProcessedBytes, DestLength: Integer);
var
  I, L, S_I, D_I: Integer;
begin
  L := Math.Min(Length(Source) - SourceIndex, Length(Dest) - DestIndex);
  Dec(L, L mod 2); // Make L even.
  ...
  for I := 0 to (L div 2) - 1 do
    begin
      Dest[D_I] := Source[S_I + 1];
      Dest[D_I + 1] := Source[S_I];
      Inc(S_I, 2);
      Inc(D_I, 2);
    end;
  DestLength := L;
...
function TUTF16BECodec.Encode(const S: WideString;
    out ProcessedChars: Integer): AnsiString;
var
  DestIndex ,I: Integer;
  W: Word;
begin
  ...
  for I := 1 to Length(S) do
    begin
      W := Ord(S[I]);
      Result[DestIndex] := AnsiChar(W shr 8);       // High byte of W.
      Result[DestIndex + 1] := AnsiChar(W and $FF); // Low byte of W.
      Inc(DestIndex, 2);
    end;
  ProcessedChars := Length(S);
end;
...
var
  ...
  LowSurrogate: array[0..1] of Byte;
begin
  if not ReadBuffer(Buf, 2) then
    begin
      C := UCS4_STRING_TERMINATOR;
      ByteCount := 0;
      Exit;
    end;
  C := (Buf[0] shl 8) or Buf[1];
  case C of
...
{ UTF-16LE                                                                     }
{                                                                              }
procedure TUTF16LECodec.Decode(const Source: TBytes; const SourceIndex: Integer;
    const Dest: TBytes; const DestIndex: Integer;
    out ProcessedBytes, DestLength: Integer);
begin
  if Assigned(Source) and Assigned(Dest) then
    begin
      DestLength := Math.Min(Length(Source) - SourceIndex, Length(Dest) - DestIndex);
      Dec(DestLength, DestLength mod 2); // Make L even.

      System.Array.Copy(Source, SourceIndex, Dest, DestIndex, DestLength);

      ProcessedBytes := DestLength;
    end
  else
    begin
      DestLength := 0;
...
function TUTF16LECodec.Encode(const S: WideString;
    out ProcessedChars: Integer): AnsiString;
var
  B: TBytes;
begin
  if Assigned(S) then
    begin
      ProcessedChars := Length(S);
      SetLength(Result, ProcessedChars * 2);
      B := System.Text.Encoding.Unicode.GetBytes(S);
      System.Array.Copy(B, 0, Result, 0, ProcessedChars * 2);
    end
  else
    begin
      ProcessedChars := 0;
...
{ TUCS4BECodec                                                                 }
{                                                                              }
procedure TUCS4BECodec.Decode(const Source: TBytes; const SourceIndex: Integer;
    const Dest: TBytes; const DestIndex: Integer;
    out ProcessedBytes, DestLength: Integer);
var Ch4     : UCS4Char;
    N, P, Q : Integer;
...
begin
  ...
  Q := DestIndex;
  C := (Length(Dest) - DestIndex) div 2;
  if (Length(Source) = 0) or (Length(Dest) = 0) or (C <= 0) then
    begin
      ProcessedBytes := 0;
      DestLength := 0;
      Exit;
  ...
  N := Length(Source) - 4;
  Dec(N, N mod 4); // Make N a devisor of 4.
  while P <= N do
    begin
      Ch4 := Source[P] * $1000000 +
             Source[P + 1] * $10000 +
             Source[P + 2] * $100 +
             Source[P + 3];
      if ((Ch4 >= $D800) and (Ch4 < $E000)) or (Ch4 > $10FFFF) then
        case FErrorAction of
          eaException :
            if Ch4 > $10FFFF then
              RaiseUnicodeCodecException(SCannotConvertUCS4, [Ch4, 'UTF-16'], P - SourceIndex)
            else
              RaiseUnicodeCodecException(SSurrogateNotAllowed, [Ch4, 'UCS-4'], P - SourceIndex);
          eaStop :
            Break;
          eaSkip :
            Inc(P, 4);
          eaIgnore :
            begin
              if L + 1 >= C then
                Break;
              Dest[Q] := Source[P + 1];     // Nevertheless change Big Endian to Little Endian ...
              Dest[Q + 1] := Source[P];
              Dest[Q + 2] := Source[P + 3];
              Dest[Q + 3] := Source[P + 2];
              Inc(Q, 4);
          ...
          eaReplace :
            begin
              if L >= C then
                Break;
              Dest[Q] := Ord(FDecodeReplaceChar) and $FF;   // Low byte.
              Dest[Q + 1] := Ord(FDecodeReplaceChar) shr 8; // High byte.
              Inc(Q, 2);
              Inc(P, 4);
              Inc(L);
            end;
        ...
        if Ch4 > $FFFF then
          begin
            if L + 1 >= C then
              Break;
            Dest[Q] := (Source[P + 1] shl 6) + (Source[P + 2] shr 2);
            Dest[Q + 1] := $D8 + (Source[P + 1] shr 2);
            Dest[Q + 2] := Source[P + 3];
            Dest[Q + 3] := $DC + (3 and Source[P + 2]);
            Inc(Q, 4);
            Inc(P, 4);
            Inc(L, 2);
          end
  ...
  DestLength := L;
  ProcessedBytes := P - SourceIndex;
end;

function TUCS4BECodec.Encode(const S: WideString;
  out ProcessedChars: Integer): AnsiString;
var
  L, M, P, Q: Integer;
...
begin
  ...
  while P <= L do
    case Ord(S[P]) of
      $D800..$DBFF: // High surrogate of Unicode character [$10000..$10FFFF]
        begin
          if P = L then // End of WideString?
            raise EConvertError.Create(SLowSurrogateNotFound);
          HighSurrogate := Ord(S[P]);
          Inc(P);
          Inc(M, 2);
          LowSurrogate := Ord(S[P]);
          case LowSurrogate of  // Low Surrogate following?
            $DC00..$DF00:
              begin
                Result[Q] := AnsiChar(0);
                Result[Q + 1] := AnsiChar((HighSurrogate - $D7C0) shr 6);
                Result[Q + 2] := AnsiChar(((HighSurrogate and $3F) shl 2) + ((LowSurrogate - $DC00) shr 8));
                Result[Q + 3] := AnsiChar(LowSurrogate and $FF);
                Inc(P);
                Inc(Q, 4);
                Inc(M, 4);
              end;
      ...
      Result[Q] := AnsiChar(0);
      Result[Q + 1] := AnsiChar(0);
      Result[Q + 2] := AnsiChar(Ord(S[P]) shr 8);   // High byte.
      Result[Q + 3] := AnsiChar(Ord(S[P]) and $FF); // Low byte.
      Inc(P);
      Inc(Q, 4);
      Inc(M, 4);
    end;
...
{ TUCS4LECodec                                                                 }
{                                                                              }
procedure TUCS4LECodec.Decode(const Source: TBytes; const SourceIndex: Integer;
    const Dest: TBytes; const DestIndex: Integer;
    out ProcessedBytes, DestLength: Integer);
var Ch4     : UCS4Char;
    N, P, Q : Integer;
      ...
      if ((Ch4 >= $D800) and (Ch4 < $E000)) or (Ch4 > $10FFFF) then
        ...
        if Ch4 > $FFFF then
          begin
            if L + 1 >= C then
              Break;
            Dest[Q] := (Source[P + 2] shl 6) + (Source[P + 1] shr 2);
            Dest[Q + 1] := $D8 + (Source[P + 2] shr 2);
            Dest[Q + 2] := Source[P];
            Dest[Q + 3] := $DC + (3 and Source[P + 1]);
            Inc(Q, 4);
            Inc(P, 4);
            Inc(L, 2);
          end
...
function TUCS4LECodec.Encode(const S: WideString;
    out ProcessedChars: Integer): AnsiString;
var
  L, M, P, Q: Integer;
  HighSurrogate, LowSurrogate: Word;
      ...
      $D800..$DBFF: // High surrogate of Unicode character [$10000..$10FFFF]
        begin
          ...
          case LowSurrogate of  // Low Surrogate following?
            $DC00..$DF00:
              begin
                Result[Q] := AnsiChar(LowSurrogate and $FF);
                Result[Q + 1] := AnsiChar(((HighSurrogate and $3F) shl 2) + ((LowSurrogate - $DC00) shr 8));
                Result[Q + 2] := AnsiChar((HighSurrogate - $D7C0) shr 6);
                Result[Q + 3] := AnsiChar(0);
                Inc(P);
                Inc(Q, 4);
                Inc(M, 4);
              end;
...
{ TUCS4_2143Codec                                                              }
{                                                                              }
procedure TUCS4_2143Codec.Decode(const Source: TBytes; const SourceIndex: Integer;
    const Dest: TBytes; const DestIndex: Integer;
    out ProcessedBytes, DestLength: Integer);
var Ch4     : UCS4Char;
    N, P, Q : Integer;
      ...
      if ((Ch4 >= $D800) and (Ch4 < $E000)) or (Ch4 > $10FFFF) then
        ...
        if Ch4 > $FFFF then
          begin
            if L + 1 >= C then
              Break;
            Dest[Q] := (Source[P] shl 6) + (Source[P + 3] shr 2);
            Dest[Q + 1] := $D8 + (Source[P] shr 2);
            Dest[Q + 2] := Source[P + 2];
            Dest[Q + 3] := $DC + (3 and Source[P + 3]);
            Inc(Q, 4);
            Inc(P, 4);
            Inc(L, 2);
          end
...
function TUCS4_2143Codec.Encode(const S: WideString;
    out ProcessedChars: Integer): AnsiString;
var
  L, M, P, Q: Integer;
  HighSurrogate, LowSurrogate: Word;
begin
  ...
  while P <= L do
    case Ord(S[P]) of
      $D800..$DBFF: // High surrogate of Unicode character [$10000..$10FFFF]
        begin
          ...
          case LowSurrogate of  // Low Surrogate following?
            $DC00..$DF00:
              begin
                Result[Q] := AnsiChar((HighSurrogate - $D7C0) shr 6);
                Result[Q + 1] := AnsiChar(0);
                Result[Q + 2] := AnsiChar(LowSurrogate and $FF);
                Result[Q + 3] := AnsiChar(((HighSurrogate and $3F) shl 2) + ((LowSurrogate - $DC00) shr 8));
                Inc(P);
                Inc(Q, 4);
                Inc(M, 4);
              end;
      ...
      Result[Q] := AnsiChar(0);
      Result[Q + 1] := AnsiChar(0);
      Result[Q + 2] := AnsiChar(Ord(S[P]) and $FF); // Low byte.
      Result[Q + 3] := AnsiChar(Ord(S[P]) shr 8);   // High byte.
      Inc(P);
      Inc(Q, 4);
      Inc(M, 4);
    end;
...
{ TUCS4_3412Codec                                                              }
{                                                                              }
procedure TUCS4_3412Codec.Decode(const Source: TBytes; const SourceIndex: Integer;
    const Dest: TBytes; const DestIndex: Integer;
    out ProcessedBytes, DestLength: Integer);
var Ch4     : UCS4Char;
    N, P, Q : Integer;
      ...
      if ((Ch4 >= $D800) and (Ch4 < $E000)) or (Ch4 > $10FFFF) then
        ...
        if Ch4 > $FFFF then
          begin
            if L + 1 >= C then
              Break;
            Dest[Q] := (Source[P + 3] shl 6) + (Source[P] shr 2);
            Dest[Q + 1] := $D8 + (Source[P + 3] shr 2);
            Dest[Q + 2] := Source[P + 1];
            Dest[Q + 3] := $DC + (3 and Source[P]);
            Inc(Q, 4);
            Inc(P, 4);
            Inc(L, 2);
          end
...
function TUCS4_3412Codec.Encode(const S: WideString;
    out ProcessedChars: Integer): AnsiString;
var
  L, M, P, Q: Integer;
  HighSurrogate, LowSurrogate: Word;
begin
  ...
  while P <= L do
    case Ord(S[P]) of
      $D800..$DBFF: // High surrogate of Unicode character [$10000..$10FFFF]
        begin
          ...
          case LowSurrogate of  // Low Surrogate following?
            $DC00..$DF00:
              begin
                Result[Q] := AnsiChar(((HighSurrogate and $3F) shl 2) + ((LowSurrogate - $DC00) shr 8));
                Result[Q + 1] := AnsiChar(LowSurrogate and $FF);
                Result[Q + 2] := AnsiChar(0);
                Result[Q + 3] := AnsiChar((HighSurrogate - $D7C0) shr 6);
                Inc(P);
                Inc(Q, 4);
                Inc(M, 4);
              end;
      ...
      Result[Q]     := AnsiChar(Ord(S[P]) shr 8);   // High byte.
      Result[Q + 1] := AnsiChar(Ord(S[P]) and $FF); // Low byte.
      Result[Q + 2] := AnsiChar(0);
      Result[Q + 3] := AnsiChar(0);
      Inc(P);
      Inc(Q, 4);
...
{ TUCS2BECodec                                                                 }
{                                                                              }
procedure TUCS2BECodec.Decode(const Source: TBytes; const SourceIndex: Integer;
    const Dest: TBytes; const DestIndex: Integer;
    out ProcessedBytes, DestLength: Integer);
var
  C, L, N, P, Q: Integer;
...
begin
  ...
  L := 0;
  N := Length(Source) - 2;
  Dec(N, N mod 2); // Make N even.
  while P <= N do begin
    W := Source[P + 1] + Source[P] * $100;
    if (W >= $D800) and (W < $E000) then // Do not accept surrogates
      case FErrorAction of
        eaException :
          RaiseUnicodeCodecException(SSurrogateNotAllowed, [W, 'UCS-2'], P - SourceIndex);
        eaStop :
          Break;
        eaSkip :
          Inc(P, 2);
        ...
        eaReplace :
          begin
            W := Ord(FDecodeReplaceChar);
            Dest[Q] := W and $FF;   // Low byte of W.
            Dest[Q + 1] := W shr 8; // High byte of W.
            Inc(P, 2);
            Inc(Q, 2);
            Inc(L, 2);
          end;
...
function TUCS2BECodec.Encode(const S: WideString;
    out ProcessedChars: Integer): AnsiString;
var
  I, J: Integer;
  W: Word;
begin
  ...
  for I := 1 to Length(S) do begin
    ...
    if (W >= $D800) and (W < $E000) then // Do not accept surrogates
      case FErrorAction of
        eaException:
          RaiseUnicodeCodecException(SSurrogateNotAllowed, [W, 'UCS-2'], I * 2);
        eaStop:
          begin
        ...
        eaIgnore:
          begin
            Result[J]     := AnsiChar(W shr 8);   // High byte of W.
            Result[J + 1] := AnsiChar(W and $FF); // Low byte of W.
            Inc(J, 2);
          end;
        eaReplace:
          begin
...
{ TUCS2LECodec                                                                 }
{                                                                              }
procedure TUCS2LECodec.Decode(const Source: TBytes; const SourceIndex: Integer;
    const Dest: TBytes; const DestIndex: Integer;
    out ProcessedBytes, DestLength: Integer);
var
  C, L, N, P, Q: Integer;
...
function TUCS2LECodec.Encode(const S: WideString;
    out ProcessedChars: Integer): AnsiString;
var
  I, J: Integer;
  W: Word;
  ...
  for I := 1 to Length(S) do begin
    ...
    if (W >= $D800) and (W < $E000) then // Do not accept surrogates
      case FErrorAction of
        ...
        eaIgnore:
          begin
            Result[J]     := AnsiChar(W and $FF); // Low byte of W.
            Result[J + 1] := AnsiChar(W shr 8);   // High byte of W.
            Inc(J, 2);
          end;
        eaReplace:
          begin
